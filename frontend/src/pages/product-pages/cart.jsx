...
const Cart = () => {
  ...
  const fetchCart = async () => {
    try {
      ...
      const validItems = (response?.cart?.items || []).filter(item => item.sticker);
  ...
  const updateQuantity = async (stickerId, newQuantity) => {
    if (!stickerId || newQuantity < 1) return;
    try {
      setUpdating(true);
      setError("");
  ...
  const calculateTotals = () => {
    ...
    const subtotal = cart.items.reduce((sum, item) => sum + (item.sticker?.price || 0) * item.quantity, 0);
  ...
  const CartItem = ({ item }) => {
    ...
    const stickerId = item.sticker?._id;

    const handleRemove = async () => {
      if (!stickerId) return;
      setIsRemoving(true);
      await removeItem(stickerId);
      setIsRemoving(false);
    };

    return (
      <div className={`relative overflow-hidden rounded-2xl p-6 backdrop-blur-xl bg-white/10 border border-white/20 hover:bg-white/20 transition-all duration-500 ${isRemoving ? "opacity-50 scale-95" : ""}`}>
        <div className="flex items-center gap-4">
          <div className="relative">
            <img
              src={item.sticker?.imageUrl}
              alt={item.sticker?.title || "Sticker Image"}
          ...
          <div className="flex-1">
            <h3 className="text-white font-semibold text-lg mb-1">{item.sticker?.title || "Unknown Sticker"}</h3>
            <p className="text-gray-300 text-sm mb-2">{item.sticker?.description || ""}</p>
            <div className="flex items-center justify-between">
              <span className="text-cyan-400 font-bold text-lg">₹{item.sticker?.price || 0}</span>
              <div className="flex items-center gap-3">
                <button
                  onClick={() => updateQuantity(stickerId, item.quantity - 1)}
        ...
        <div className="mt-4 pt-4 border-t border-white/10 flex justify-between items-center">
          ...
          <span className="text-white font-bold">₹{(item.sticker?.price || 0) * item.quantity}</span>
        </div>
      </div>
    );
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {cartItems.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-300 mb-8">Start adding some awesome stickers to your cart!</p>
            <Link to="/" className="inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-cyan-500 to-purple-600 text-white font-bold rounded-xl hover:from-cyan-600 hover:to-purple-700 transition-all duration-300 transform hover:scale-105">
              Browse Stickers
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              {cartItems.map((item, index) => <CartItem key={item.sticker?._id || index} item={item} />)}
            </div>
